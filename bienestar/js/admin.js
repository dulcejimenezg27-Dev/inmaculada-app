(() => {
  ...
  const CFG = window.BIENESTAR_CONFIG || { allowedEmails: [] };
  ...
  function isBienestarEmail(email) {
    if (FB?.isBienestarEmail) return FB.isBienestarEmail(email);
  ...
  function autorSnapshot() {
    if (FB?.buildAutorFromPerfil) {
      ...
      if (!a.licenciatura && !a.cargoLabel) {
        a.licenciatura = "Psicóloga";
        a.cargoLabel = "Psicóloga";
      }
      return a;
    }
    return {
      ...
      nombreCompleto: "Psicóloga",
      licenciatura: currentPerfil?.licenciatura || "Psicóloga",
  ...
  function applyRoleUI() {
    ...
    if (roleLabel) roleLabel.textContent = nombre || "Inmaculada Bienestar";
    if (emailLabel) {
  ...
  function openPerfilModal({ required = false } = {}) {
    ...
    form.licenciatura.value = currentPerfil?.licenciatura || "Psicóloga";
    ...
    if (lead) {
      lead.textContent = required
        ? "Antes de publicar, completa tu nombre. Así te verán en Bienestar."
        : "Así aparecerás en las publicaciones de Bienestar.";
    }
  ...
  async function loadPosts() {
    ...
    if (!FB?.fetchBienestar) return;
    try {
      if (FB.whenReady) await FB.whenReady();
      const items = await FB.fetchBienestar();
  ...
  function renderPosts() {
    ...
    list.innerHTML = items
      ...
      <article class="admin-item">
        ${
          C?.autorMetaHtml
            ? C.autorMetaHtml(p.autor || { nombreCompleto: "Bienestar" }, formatFecha(p.fecha), p.categoria)
  ...
  async function enterAs(user) {
    ...
    FB = window.InmaculadaFirebase || FB;
    if (!isBienestarEmail(user.email)) {
      try {
        await FB.signOut();
      } catch {
        /* ignore */
      ...
      setLoginBusy(false);
      showLoginError(
        "Este correo no está autorizado para Bienestar. Pide al colegio que lo agregue o use el correo de psicología."
      );
      showApp(false);
      return;
    }
  ...
  document.getElementById("form-perfil")?.addEventListener("submit", async (e) => {
    ...
    try {
      currentPerfil = await FB.savePerfil({
        ...
        licenciatura: form.licenciatura.value || "Psicóloga",
  ...
  /* PWA install (same pattern as docentes) */
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.register("./sw.js", { scope: "./" }).catch(() => {});
  }

  const INSTALL_DISMISS_KEY = "inmaculada_bienestar_install_dismissed";
  const INSTALL_DONE_KEY = "inmaculada_bienestar_installed";
  ...
  function shouldOfferInstall() {
    if (isStandalone || localStorage.getItem(INSTALL_DONE_KEY) === "1") return false;
  ...
  btnInstall?.addEventListener("click", async () => {
    ...
    if (isAndroid) document.getElementById("modal-android-install")?.showModal();
    else document.getElementById("modal-win-install")?.showModal();
  });
